...
Sprawdzona została poprawność działania systemu w zintegrowanym środowisku, gdzie zostały one przeprowadzone niemal od początku do końca. Nie były one wykonane w 100\% od początku do końca z uwagi ukończenie pracy w okresie zimowym, kiedy nie są rejestrowane nagrania nietoperzy a jedynie niekiedy nagrania szumów. Z tego powodu testy te zostały podzielone na dwa elementy - poprawność pobierania dźwięków oraz poprawność funkcjonalności od pobrania dźwięku, poprzez identyfikację głosu nietoperza po zapis rekordu w bazie i wizualizacji go w Aplikacji Użytkownika.
