...
\begin{lstlisting}[language=C,caption={Polecenie rsync pobierające dane z pewnego okresu czasu}, label={lst:functional_component}]
...
\section{Synchronizacja danych pomiędzy systemem SMART a bazą danych i aplikacją}
Do synchronizacji danych posłużył watchdog, który został zaimplementowany jako mikroserwis, wystawiający API dla NodeJS. Watchdog monitoruje stan folderu opisanego w poprzednim punkcie i w przypadku pojawienia się nowego rekordu do NodeJS trafia plik w formacie JSON z wszelkimi informacjami przechowywanymi przez bazę danych i prezentowanych w interfejsie użytkownika, takimi jak: data, godzina, numer turbiny itd.

 >>>>>>>>>>>>>>>  JAK TO ZAPREZENTOWAĆ  ????

\section{Przekazanie danych do aplikacji}
Za pośrednictwem watchdoga dane z systemu SMART trafiają do bazy danych z wykorzystaniem serwera w NodeJS i interfejsu w ReactJS.
